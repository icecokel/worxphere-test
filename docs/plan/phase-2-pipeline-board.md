...
- [ ] `/`에서 정해진 순서의 5개 컬럼과 현재 단계에 맞는 지원자 카드를 표시한다.
- [ ] 카드에 이름·직무·지원일·현재 단계를 표시하고, 컬럼 인원수와 실제 카드 수가 일치한다.
- [ ] 로딩, 조회 실패와 재시도, 전체 빈 상태, 컬럼별 빈 상태를 서로 구분해 표시한다.
- [ ] 좁은 화면에서는 컬럼 너비를 유지한 채 가로 스크롤되고 shadcn/ui 기본 스타일을 유지한다.
- [ ] Phase 검증과 `PROMPTS.md`의 `[pipeline-board]` 기록이 완료됐다.
