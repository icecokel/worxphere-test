...
- 정규 전이는 `서류검토 → 면접 → 처우협의 → 최종합격`으로 한정하며, 중간 단계 건너뛰기와 일반적인 이전 단계 변경을 제공하지 않는다.
...
- 실행 취소는 동일한 단계 저장 PATCH에 `undo: true`를 명시해 바로 이전 비종료 단계로만 복구한다.
...
- 일반 단계 저장 요청 본문: `{ "stage": "INTERVIEW" }`
- 실행 취소 요청 본문: `{ "stage": "DOCUMENT_REVIEW", "undo": true }`. 바로 이전 비종료 단계 복구만 허용하며 일반 이전 단계 요청과 종료 단계 실행 취소는 `400`으로 거부한다.
- 최종합격 지원자를 불합격으로 변경하려는 요청은 `400` 응답으로 거부한다.
- 모든 mock API 요청에는 `200~800ms` 지연을 적용한다.
- 계약과 도메인 규칙을 통과한 유효한 요청에만 약 `15%` 확률의 임의 `500`을 적용한다.
- 목록 조회는 쿼리 검증 `400` 뒤에 임의 `500`을 판정한다.
...
- [ ] 중간 단계 건너뛰기와 일반적인 이전 단계 변경이 제공되지 않고, 불합격 처리는 `서류검토`, `면접`, `처우협의`에서만 제공된다.
- [ ] 확인을 취소하면 카드와 API 요청이 변경되지 않는다.
- [ ] 저장 성공 후 새로고침해도 이동한 단계가 유지된다.
- [ ] 저장 실패 시 카드가 원래 컬럼으로 복구되고 실패 메시지가 표시된다.
- [ ] 조회 실패 상태에서 `다시 시도`가 동작한다.
- [ ] 잘못된 목록 조건, 존재하지 않는 지원자, 잘못된 요청 본문과 허용되지 않은 단계 전이가 임의 `500`보다 먼저 `400/404`로 응답한다.
- [ ] 부분 조회 실패와 재시도 중에도 이미 표시된 지원자 카드가 유지된다.
- [ ] 전체 데이터 없음, 검색 결과 없음, 빈 컬럼이 구분되어 표시된다.
- [ ] 200건 이상의 지원자에서 이름·직무 조건 변경 뒤 첫 목록 요청과 마지막 응답 뒤 화면 반영이 각각 `100ms` 이내이고, 클라이언트 반영 구간의 `50ms` 이상 Long Task가 없다.
