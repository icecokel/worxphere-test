...
- `불합격 처리` 섹션: 이미 불합격이 아닌 모든 상태에서 별도 위험 액션을 표시한다.
- 다음 단계가 없거나 이미 불합격이면 해당 액션을 비활성화하거나 안내한다.
- 액션 버튼을 누르면 지원자 이름, 이전 단계, 대상 단계와 알림 가능성을 확인한다.
- 확인을 취소하면 화면 상태와 API 요청을 만들지 않는다.
...
3. 지원자 이름, 이전 단계, 대상 단계와 알림 가능성이 포함된 확인창에서 변경을 확정한다.
...
- 중간 단계 건너뛰기와 이전 단계 변경은 허용하지 않는다.
- 불합격 처리는 이미 불합격이 아닌 모든 상태에서 가능하다.
...
- 변경 확정에는 브라우저 기본 확인창을 사용하며 별도 다이얼로그 의존성을 추가하지 않는다.
